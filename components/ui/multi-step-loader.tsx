    ...
    <div className="flex relative justify-start max-w-xl mx-auto flex-col gap-2">
      {loadingStates.map((loadingState, index) => {
        ...
        return (
          <motion.div
            key={index}
            className={cn("text-left flex gap-4 items-center")}
          ...
          >
            <div>
              {index > value && (
                <CheckIcon className="text-[#32CD32]/40" />
              )}
              {index <= value && (
                <CheckFilled
                  className={cn(
    ...
    <AnimatePresence mode="wait">
      {loading && (
        <motion.div
          ...
          style={{
            background: 'linear-gradient(180deg, rgba(50, 205, 50, 0.1) 0%, rgba(0, 0, 0, 0.9) 100%)',
          }}
        >
