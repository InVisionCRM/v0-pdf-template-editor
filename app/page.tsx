...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-black overflow-hidden">
          ...
          <DropdownMenuContent align="end" className="w-56 bg-black border border-[#F1F20E]">
            <DropdownMenuLabel className="border-b border-[#F1F20E]/20">
              <div className="flex flex-col space-y-1">
                ...
                <p className="text-xs leading-none text-[#F1F20E]">{session?.user?.email}</p>
              </div>
            </DropdownMenuLabel>
            <DropdownMenuSeparator className="bg-[#F1F20E]/20" />
            <DropdownMenuItem 
              onClick={() => signOut()}
              className="text-white hover:text-white hover:bg-[#F1F20E]/20 cursor-pointer"
            >
              Log out
            </DropdownMenuItem>
          </DropdownMenuContent>
      ...
      <div className="fixed left-8 top-8 z-50">
        <div className="relative">
          <Image
            ...
            priority
          />
          <h1 className="text-lg sm:text-xl md:text-2xl font-bold mt-2 text-[#F1F20E]">In-Vision Construction</h1>
          <p className="text-sm sm:text-base text-gray-500">Contracts</p>
        </div>
      </div>

      {/* Main Content Container - Adjusted to account for logo */}
      <div className="w-full fixed left-1/2 -translate-x-1/2 top-[350px] z-50">
        <div className="max-w-7xl w-full mx-auto px-4">
          {/* Tips Section */}
          <div className="w-auto relative left-1/2 -translate-x-1/2 top-[1px] z-50">
            <RandomTip />
          </div>

          {/* Lead Search Section */}
          <div className="mb-2 bg-black bg-opacity-25 py-2 px-4 rounded-lg border border-[#F1F20E] border-opacity-25 max-w-lg mx-auto">
            <h2 className="text-sm font-semibold text-[#F1F20E] inline-block mr-2">Search Lead:</h2>
            <LeadSearch onLeadSelect={handleLeadSelect} selectedLead={selectedLead} />
          </div>

          {/* Selected Lead Info */}
          {selectedLead && (
            <div className="mb-2 bg-black bg-opacity-50 py-3 px-4 rounded-lg border border-[#F1F20E] border-opacity-25 max-w-lg mx-auto">
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                  <div className="flex items-center">
                    <span className="font-semibold text-[#F1F20E] mr-2">Name:</span>
                    <span className="text-white">{selectedLead.firstName} {selectedLead.lastName}</span>
                  </div>
                  <div className="flex items-center">
                    <span className="font-semibold text-[#F1F20E] mr-2">Phone:</span>
                    <span className="text-white">{selectedLead.phone || 'N/A'}</span>
                  </div>
                  <div className="flex items-center">
                    <span className="font-semibold text-[#F1F20E] mr-2">Email:</span>
                    <span className="text-white">{selectedLead.email || 'N/A'}</span>
                  </div>
                  <div className="flex items-center">
                    <span className="font-semibold text-[#F1F20E] mr-2">Address:</span>
                    <span className="text-white">{selectedLead.address || 'N/A'}</span>
                  </div>
                </div>
                <Button
                  variant="ghost"
                  className="h-6 px-2 text-[#F1F20E] hover:text-[#F1F20E] hover:bg-[#F1F20E]/10 ml-2"
      ...
      <div className="w-full fixed left-1/2 -translate-x-1/2 top-[200px] z-40">
        <div className="max-w-7xl w-full mx-auto px-4">
          {/* Contract Types Grid */}
          <div className="grid grid-cols-4 gap-1">
            <Card className="bg-transparent border-none shadow-none">
              <CardContent className="p-2">
                <Button
                  className="w-full btn-gradient py-6 px-4 text-md whitespace-normal text-black h-auto hover:scale-105"
